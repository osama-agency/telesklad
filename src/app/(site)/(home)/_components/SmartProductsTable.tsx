...
// Улучшенный компонент кнопок корзины согласно UX рекомендациям
...
function SmartProductsTableContent() {
  ...
  const [summary, setSummary] = useState<AnalyticsResponse['summary'] | null>(null);
  ...
  // Функция для работы с кэшем курса валют
  const getCachedExchangeRate = (): number | null => {
    try {
      const cached = localStorage.getItem('exchangeRate_TRY');
      if (cached) {
  ...
  const setCachedExchangeRate = (rate: number) => {
    try {
      ...
      localStorage.setItem('exchangeRate_TRY', JSON.stringify(cacheData));
      console.log('💾 Cached exchange rate:', rate);
    } catch (error) {
      console.error('Error caching exchange rate:', error);
    }
  };

  const loadExchangeRate = async (): Promise<number | null> => {
    ...
    try {
      ...
      if (data.success && data.data.rate) {
        const rate = data.data.rateWithBuffer; // Используем курс с буфером
        setCachedExchangeRate(rate);
        console.log('🌐 Loaded fresh exchange rate:', rate);
        return rate;
      }
    } catch (error) {
  ...
  const loadAnalytics = async () => {
    try {
      ...
      // Загружаем курс валют параллельно с аналитикой
      const ratePromise = loadExchangeRate();
      
      // Рассчитываем период в днях
      if (!dateRange.from || !dateRange.to) {
        throw new Error('Не выбран период анализа');
      }
      
      const diffTime = dateRange.to.getTime() - dateRange.from.getTime();
      const diffDays = Math.max(1, Math.ceil(diffTime / (1000 * 60 * 60 * 24))); // Минимум 1 день
      
      const response = await fetch(`/api/products/analytics?period=${diffDays}`);
      ...
      // Ждем загрузки курса валют и устанавливаем его
  ...
  const createPurchase = async (items: CartItem[], totalTRY: number, totalRUB: number, supplierName: string = 'Поставщик Турция', notes: string = '') => {
    try {
      ...
      alert(`Закупка успешно создана! ID: ${result.data.id}`);
    } catch (error) {
      console.error('Ошибка создания закупки:', error);
      alert(error instanceof Error ? error.message : 'Ошибка при создании закупки');
    }
  };
                ...
                <tr key={product.id} className="border-t border-stroke hover:bg-gray-50 dark:border-dark-3 dark:hover:bg-gray-800 transition-colors">
                  <td className="px-4 py-4">
                    <div>
                      ...
                      {exchangeRate ? (
                        <div className="text-xs space-y-0.5">
                          <div className="text-purple-600 dark:text-purple-400 font-medium">
                            🏷️ {product.avgSalePrice.toLocaleString()} ₽ (продажа)
                          </div>
                      ...
                      <div className="text-xs text-gray-400">Остаток: {product.currentStock} + {product.inTransitQuantity} в пути</div>
                    </div>
                  </td>
                  <td className="px-4 py-4">
