...
import { AdaptiveIconComponent } from '@/components/webapp/AdaptiveIconComponent';
...
export function TelegramDesignHeaderAdaptive() {
  const pathname = usePathname();
  const { favoritesCount, hasFavorites } = useFavorites();
  const { user, isAuthenticated } = useTelegramAuth();
  const { impactLight } = useTelegramHaptic();
  const { theme, themeConfig, isDark } = useTelegramTheme();
  
  // State для прокрутки и устройства
  ...
  // Получение Telegram device capabilities
  useEffect(() => {
  ...
  // Динамические стили на основе адаптивной темы
  ...
  }), [themeConfig, deviceCapabilities.safeAreaInset]);
  ...
  return (
    <header 
      className={`telegram-design-header adaptive-theme theme-${theme} ${isScrolled ? 'scrolled' : ''} ${deviceCapabilities.isLowEnd ? 'low-end' : ''}`}
      style={headerStyles}
      role="banner"
      ...
      <div className="header-container">
        ...
        <nav className="header-actions" role="navigation" aria-label="Основные действия">
          ...
          <Link 
            ...
            className={`header-action-button adaptive-button ${isActive('/webapp/favorites') ? 'active' : ''}`}
          ...
          <Link 
            href="/webapp/profile" 
            className={`header-action-button profile-button adaptive-button ${isActive('/webapp/profile') ? 'active' : ''}`}
            onClick={handleButtonClick}
            ...
            <div className="header-action-icon">
              {user?.photo_url ? (
                <img 
                  ...
                  loading="lazy"
                  style={{
                    border: `2px solid ${isActive('/webapp/profile') ? themeConfig.colors.primary : themeConfig.colors.surface}`
                  }}
                />
              ) : (
                <AdaptiveIconComponent 
