...
export function AdaptiveIconComponent({ name, size = 24, className = "" }: IconProps) {
  const { theme, themeConfig } = useTelegramTheme();
  ...
  const icons: Record<string, JSX.Element> = {
    ...
    "cart-empty": (
      ...
      >
        <g>
          <path 
            d="m19.976 134.951h182.048v30h-182.048z" 
            fill="#606075" 
            transform="matrix(.692 -.722 .722 .692 -74.057 126.284)"
          />
          <path 
            d="m386 58.927h30v182.048h-30z" 
            fill="#515165" 
            transform="matrix(.722 -.692 .692 .722 7.782 319.262)"
          />
          <path 
            d="m24.652 258.553 40.649 227.797h190.699l18-133.886-18-93.911z" 
            fill={theme === 'dark' ? '#5c8f00' : '#92e200'}
            className="cart-primary"
          />
          <path 
            d="m487.348 258.553h-231.348v227.797h190.699z" 
            fill={theme === 'dark' ? '#1f5415' : '#00c100'}
            className="cart-secondary"
          />
          <path 
            d="m0 203.147v80.813h256l13.72-39.407-13.72-41.406z" 
            fill={theme === 'dark' ? '#8fb347' : '#d7f369'}
            className="cart-accent"
          />
          <path 
            d="m256 203.147h256v80.813h-256z" 
            fill={theme === 'dark' ? '#5c8f00' : '#92e200'}
            className="cart-primary"
          />
          <path 
            d="m199.224 318.553h30v115h-30z" 
            fill={theme === 'dark' ? '#8fb347' : '#d7f369'}
            className="cart-accent"
          />
          <path 
            d="m282.776 318.553h30v115h-30z" 
            fill={theme === 'dark' ? '#5c8f00' : '#92e200'}
            className="cart-primary"
          />
          <path 
            d="m366.328 318.553h30v115h-30z" 
            fill={theme === 'dark' ? '#5c8f00' : '#92e200'}
            className="cart-primary"
          />
          <path 
            d="m115.672 318.553h30v115h-30z" 
            fill={theme === 'dark' ? '#8fb347' : '#d7f369'}
            className="cart-accent"
          />
          <circle 
...
// Обратная совместимость - экспортируем старый компонент с адаптивной логикой
export function IconComponent(props: IconProps) {
  return <AdaptiveIconComponent {...props} />;
}
