...
import { RequestDeduplication } from '@/lib/utils/request-deduplication';
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!tgId) {
      return NextResponse.json({ error: 'tg_id parameter is required' }, { status: 400 });
    }

    // 🚀 ДЕДУПЛИКАЦИЯ + КЭШИРОВАНИЕ
    return await RequestDeduplication.dedupe(`favorites:${tgId}`, async () => {
      ...
      const transformedResult = favorites
      ...
      console.log(`✅ Returning ${transformedResult.length} favorites for user ${user.id} (tg_id: ${tgId})`);
      console.log('Sample product data:', transformedResult[0]);
